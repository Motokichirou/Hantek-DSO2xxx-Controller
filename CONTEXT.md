...
**СЛЕДУЮЩИЙ ШАГ:**
1. **Hardware-verify фундамента (за пользователем):** подключить DSO2D15, запустить `.venv/Scripts/python.exe scripts/hw_smoke_foundation.py`, прислать вывод (*IDN?, readback, set+readback). По нему калибруем литералы (run/stop/single ещё не реализованы — их нет в мануале, отдельная hardware-задача, spec §11 п.10).
2. **Следующий план — Layer 3: `waveform/`** (чанковый декодер `WAVeform:DATA:ALL?`, risk §5 — калибровка сэмплов→вольты/время на железе). Затем engine + минимальный gui (первый live-дисплей).
