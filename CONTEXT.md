...
**СЛЕДУЮЩИЙ ШАГ — Layer 4: engine-цикл (фоновый QThread: arm→poll STATus→read_frame→decode→эмит) + минимальный gui (pyqtgraph plot + connect + run/stop)** → первый live-дисплей обоих каналов. GUI — пиксель-в-пиксель по `design/` (см. §6/§7). run/stop/single/autoscale всё ещё не реализованы (нет в мануале, spec §11 п.10) — управление сбором уточнить на железе в engine-слое.
