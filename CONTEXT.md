...
**СЛЕДУЮЩИЙ ШАГ — план Layer 3: `waveform/`** (чанковый декодер `WAVeform:DATA:ALL?`, risk §5 — калибровка сэмплов→вольты/время на железе по эталону). Затем engine + минимальный gui (первый live-дисплей обоих каналов).

(run/stop/single/autoscale ещё не реализованы — их нет в мануале, отдельная hardware-задача, spec §11 п.10. Кодировка/байт-на-сэмпл waveform — главный нерешённый риск, калибруется в Layer 3.)
