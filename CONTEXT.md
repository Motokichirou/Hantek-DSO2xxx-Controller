...
### Сессия 8 — 2026-06-28 (дизайн-пасс ч.1+ч.2 + перф + decode-старт)
Большой блок. Между сессиями 7 и 8 (не записано отдельно, но в `main`/ветке): добавлены **Sweep** (multi-capture), **Save/экспорт + пресеты** (`io/`), **SCPI-терминал** (нижний док, кнопка SCPI), **README + MIT LICENSE**, опубликовано на GitHub (`Motokichirou/Hantek-DSO2xxx-Controller`, workflow feature-branch→PR→merge). Эта сессия — на ветке **`feature/design-pass`**.
