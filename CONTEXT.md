...
5. **ТЕКУЩЕЕ СОСТОЯНИЕ (2026-06-27, сессия 8):** рабочее `python -m hantek_dso2d15.app` — live-дисплей + **Scope-таб полностью** (Вертикаль/Горизонталь/Триггер/Измерения/Acquire/Math-FFT/Курсоры/Дисплей) + **таб Generator (DDS)** + **таб Sweep / Multi-capture** + тулбар **Save / PNG / Presets / LOG / SCPI** + нижний **SCPI-терминал** + honest **timebase-зум**. **1231 passed**, всё в `main`, дерево чистое. **Опубликовано на GitHub** (`Motokichirou/Hantek-DSO2xxx-Controller`, MIT, README), работаем по **workflow A: ветка → PR → merge → sync**.
...
**Следующее: п.5 — дизайн-пасс** пиксель-в-пиксель по `design/` (см. §6 ниже + `design/design_handoff_dso2d15_ui/` README/прототип/4 референса; SCPI-привязка строго по frozen reference, НЕ по README дизайна).
