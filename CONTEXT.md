...
5. **ТЕКУЩЕЕ СОСТОЯНИЕ (конец дня 2026-06-27):** рабочее приложение `python -m hantek_dso2d15.app` — **live-дисплей ОБОИХ каналов** (CH1 жёлтый, CH2 зелёный), ~3 fps. Готовы слои: transport · scpi (channel/timebase/acquire/trigger-edge) · waveform (PRIVate-декодер, оба канала) · engine (QThread) · gui (минимальный). **422 passed.** Все коммиты в `main`, дерево чистое. Узловой риск §5 и CH2 — решены.

   **ПЛАН НА ЗАВТРА (приоритеты пользователя):**
   1. **Поискать ещё `PRIVate:*` команды** — раскрытие `PRIVate:WAVeform:DATA:ALL?` намекает, что есть и другие скрытые префиксы (возможно быстрый bulk-readout, доп. данные). Прозондировать на железе.
   2. **Доделать UI** — правый док панелей по `design/design_handoff_dso2d15_ui/` (Вертикаль с probe/scale/offset/coupling по каждому каналу — нужно для корректного масштаба CH2 с щупом ×100; Горизонталь; Триггер; Acquire; Измерения; Math; Курсоры; Дисплей), статус-бар, маркеры. Пиксель-в-пиксель.
   3. **Установщик** — собрать MSI/EXE (напр. PyInstaller + inno setup / MSIX), подтягивающий все зависимости (PySide6/pyvisa/numpy + venv). Учесть требование NI-VISA (или встроить pyvisa-py?).
   4. **Публикация** в общий доступ (репозиторий/релиз).
