...
>
> **Последнее обновление: 2026-06-28 (сессия 8, дизайн-пасс + перф + decode-старт).** Текущее состояние и план — §0.5; журнал — §14.
...
5. **ТЕКУЩЕЕ СОСТОЯНИЕ (2026-06-28, сессия 9):** рабочее `python -m hantek_dso2d15.app` (или ярлык/`Run-Hantek-DSO2D15.bat`) — live-дисплей + **Scope-таб** + **Generator** + **Sweep** + **Save/пресеты** + **SCPI-терминал** + **дизайн-пасс** (тема `gui/theme.py`, аккордеон+summary-чипы, segmented, плитки форм генератора, маркеры триггера+drag, богатый бейдж со статистикой измерений) + **декодирование шин** (UART/SPI/I²C: клиентские декодеры + панель ДЕКОД + оверлей символов). **1372 passed.** **Опубликовано на GitHub** (`Motokichirou/Hantek-DSO2xxx-Controller`, MIT, README), **workflow A** (ветка→PR→merge→sync). Дизайн-пасс + перф + decode **влиты в `main` через PR #4**; дерево чистое.

   **ПЕРФ (важно):** опрос измерений **отвязан** от частоты кадров — round-robin порциями (`_measure_batch`) + каденция (`_measure_poll_period` ~0.2с) + кэш + троттлинг UI-эмиссии; интервал сбора 50мс (был 5мс — провоцировал десинк USBTMC → «кадр не собран за 512 пакетов»). Бейдж-ридаут дедуплицируется (релейаут только при изменении). Диагностика в статус-баре: `rd <мс>/<пакетов> · ms <мс>`, FPS по скользящему окну. **Диагноз скорости окончателен:** ~**480 мс на первый запрос кадра** (прибор готовит кадр; не зависит от памяти 4К/развёртки/числа пакетов — подтверждено SCPI-логом: 2 пакета по 4000 байт, первый запрос ~480мс, второй ~150мс) → ~1.4 FPS **потолок железа**, софтом/прошивкой не лечится. Связь прибора сделана под автоматизацию/глубокий захват/документирование, не под быстрый live.

   **Найдены дампы прошивки** прибора (в `tools/`, gitignore — не в репо): FULL/EE/FPGA/Calibration/Model/Software `.upk`+tar. Отдельная песочница-трек для реверса (быстрый readout не гарантирован) — по запросу.

   **РЕШЕНО РАНЕЕ:** (а) поиск `PRIVate:*` исчерпан — есть ТОЛЬКО `PRIVate:WAVeform:DATA:ALL?`; скриншота нет; `:SETUp:ALL?` query-only (парной set-команды нет → восстановление пресета через наш JSON, не raw-блоб). (б) Решение: **функционал сначала, дизайн-пасс (пиксель-в-пиксель по `design/`) — потом** (сейчас как раз пора дизайн-пасса).
   ...
   **ПЛАН:**
   ...
   2. ✅ ~~Generator (DDS)~~ — СДЕЛАНО (сессия 7).
   3. ✅ ~~Sweep / Multi-capture~~ — СДЕЛАНО (сессия 8). zoom-strip на графикуле — ещё нет (хвост).
   4. ✅ ~~Save/экспорт (CSV/NPY/HDF5/PNG) + пресеты (JSON) + SCPI-терминал + SCPI-логгер~~ — СДЕЛАНО (сессия 8).
   5. **Дизайн-пасс** пиксель-в-пиксель по `design/` (токены, segmented-тогглы, аккордеон 8 панелей, маркеры, статус-бар). ← СЛЕДУЮЩЕЕ
   6. **Установщик** EXE (PyInstaller + Inno Setup) — учесть **NI-VISA**. **Релиз на GitHub**.
   - Отложено (хвосты): zoom-strip; ARB-загрузка произвольных форм (`:DDS:ARB:DAC16:BIN`, 4096 точек); быстрый readout (research-спайк); привязка зума к триггеру; декодирование шин в триггере.
...
### Сессия 9 — 2026-06-28 (дизайн-пасс ч.1+ч.2 + перф + декодирование шин)
Большой блок на ветке `feature/design-pass` (влита в `main` через **PR #4**). Функциональный фундамент (Sweep/Save/терминал/GitHub) — в записи сессии 8 ниже.

**Дизайн-пасс ч.1** (`3a7dc5f`): `gui/theme.py` (токены: палитра, шрифты Segoe UI + JetBrains Mono, `rgba()`-хелпер — **Qt QSS не понимает 8-значный hex `#RRGGBBAA`**, трактует как `#AARRGGBB`; CH2 → cyan `#23C8E6`; `CH_COLORS`), глобальный QSS; сетка-графикуль по эталону + субделения; **маркеры триггера** (пунктир уровня + треугольник по цвету источника + T-маркер) с **drag уровня мышью** (`triggerLevelChanged` → `apply_setting`); бейдж статуса; `gui/segmented.py SegmentedControl` + `gui/accordion.py CollapsibleSection` (TDD); Scope-док → аккордеон; Sweep/Slope/Mode → segmented.

**Дизайн-пасс ч.2 + перф** (`bb82929`): **богатый бейдж** графика — строки каналов (V/дел+offset)/триггер/развёртка/**math**/**курсоры** + **таблица статистики измерений** (cur/avg/max/min/std/rms/cnt, цвет по каналу, жёсткая моноширинная вёрстка, клиентский накопитель `measure_stats.py` — прибор статистик не отдаёт; кнопка «Сброс ст.», сброс на Run/Single). **Перф-отвязка измерений** (round-robin+каденция+кэш — см. §0.5). Интервал 5→50мс. Диагностика `rd/pk/ms` + честный FPS. Лёгкая обратная связь коннекта. `Run-Hantek-DSO2D15.bat` (+`*.lnk` в `.gitignore`).

**Диагноз скорости (окончательный, по SCPI-логу):** не память, не развёртка, не число пакетов — **~480 мс на первый запрос кадра** (прибор готовит кадр), второй пакет ~150мс → ~1.4 FPS **потолок железа DSO2000**, не лечится. Детали — §0.5 п.5.

**Дизайн-пасс (хвост):** Coupling в карточке канала → segmented (акцент по цвету канала); **генератор: тип волны → плитки** (`gui/waveform_tiles.py WaveformPicker` — нарисованные QPainter'ом глифы синус/прямоугольник/пила/экспонента/шум/DC + ARB); **заголовки аккордеона по макету** (letter-spacing .7px через QFont — QSS его не поддерживает; шеврон 10px; **summary-чип** JetBrains Mono 600 10px справа — `CollapsibleSection.set_summary(text,color)`; пример: Триггер «Edge · CH1» жёлтый, Измерения — счётчик зелёный, Math — операция magenta). **Аккордеон: QSplitter отвергнут** (плохо дружит с collapse — либо растягивает одну секцию на весь экран, либо оставляет дыры в слотах) → вернулись к QVBoxLayout+addStretch (естественная высота по контенту).

**Функционал-трек (#2, TDD без железа — у пользователя нет на чём гонять шины):** **UART-декодер** `waveform/decode_uart.py` — чистый клиентский декодер из сэмплов (UART/SPI/IIC в SCPI — только триггер-setup, декодированных данных прибор НЕ отдаёт). 16 TDD-тестов (байты, LSB/MSB, 5-8 бит, parity EVEN/ODD, framing, инверсия, шум, обрывы). Делегирован сабагенту + ревью. UI декодера ещё нет.

**Тесты: 1337 passed.** Ветка `feature/design-pass`.

**Следующее:** (1) хвост #2 — **SPI** и **I²C** декодеры (та же TDD-схема) + **Decode-UI** (выбор протокола/параметров + оверлей символов на графике поверх осциллограммы по времени); (2) живые summary-чипы для остальных секций (Вертикаль «CH1·CH2», Триггер из реального состояния); (3) собрать PR ветки `feature/design-pass` → `main`; (4) пользователь нашёл **прошивки** прибора — при желании отдельная ветка-песочница для реверса (быстрый readout не гарантирован).
### Сессия 8 — 2026-06-27 (Sweep · Save/экспорт · пресеты · SCPI-терминал · публикация на GitHub)
Закрыт весь функциональный план (пп.3-4). Каждый блок: контракт → file-disjoint TDD-юниты → независимое ревью → интеграция → железо. **1231 passed.**
- **Sweep / Multi-capture** (`engine/sweep.py` чистый `SweepRunner` + `gui/panels/sweep.py` + worker-слот `run_sweep` с потокобезопасной отменой `threading.Event`): свип параметра (частота/амплитуда генератора) Start→Stop по Step, dwell, захват каждого кадра в файл. На железе — 41 кадр в CSV.
- **io-экспорт** (`io/export.py`): CSV/NPY/HDF5 + `capture_filename`.
- **Save / Screenshot / Presets** (тулбар): Save кадра (export_frame), PNG (грабим наш график — у прибора скриншот-команды нет), Presets JSON (`io/presets.py`: capture/apply/save/load + `SnapshotScope` для ресинка панелей без I/O). **HW-баг найден probe'ом:** `:DDS:BURSt:SWITch` гасит выход генератора → `apply_preset` ставит `dds.output` ПОСЛЕДНИМ (устойчиво к порядку ключей). Регрессионный тест полноты `PRESET_PATHS` (поймал недостающий `trigger.holdoff`).
- **SCPI-логгер** (`transport/scpi_log.py` + хук в visa_transport): кнопка LOG, файл с ротацией 10 МБ, осциллограмма свёрнута. **Им же диагностировали** «50нс=2нс одинаковы» (баг был в рендере, не в приборе) и run/stop-мелькание (прибор сам морозит сбор).
- **SCPI-терминал** (`gui/scpi_terminal.py` + `scope.transport` + worker `send_command`/`commandResult`): нижний док, кнопка SCPI, история ↑/↓, query/write+OK/ERR.
- **Публикация:** репо `Motokichirou/Hantek-DSO2xxx-Controller` (PUBLIC, MIT, двуязычный README). **gh CLI** залогинен. **Workflow A:** фиче-ветка → PR → merge → `git checkout main && git pull` → удалить влитую ветку. PR #1 (доки) и #2 (терминал) влиты. ⚠️ `/code-review ultra` (облачное) требует ОТДЕЛЬНОЙ авторизации GitHub-App в настройках Claude (локальный gh-токен ему не передаётся).

**Следующее: п.5 — дизайн-пасс** пиксель-в-пиксель по `design/` (см. §6 ниже + `design/design_handoff_dso2d15_ui/` README/прототип/4 референса; SCPI-привязка строго по frozen reference, НЕ по README дизайна). *(Выполнено — см. сессию 9 выше.)*
