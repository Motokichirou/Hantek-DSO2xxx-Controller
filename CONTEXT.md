...
**⚠️ КВИРК ПРОШИВКИ (HARDWARE-VERIFIED 2026-06-27, при сборке GUI):** `:WAVeform:DATA:ALL?` на DSO2D15 fw 3.0.0 **возвращает только CH1**, продублированный по числу включённых каналов. `:WAVeform:SOURce CHANnel2` НЕ переключает (подтверждено offset-подписью при 5В/дел). CH2 через этот SCPI недостижим — нужен **реверс-инжиниринг родного софта** (захват USBTMC-трафика, напр. Wireshark+USBPcap). Интерим: контроллер дедуплицирует одинаковые пакеты → GUI показывает CH1 (без фейкового дубля). Forward-совместимо. Также добавлена защита ридера от коротких пакетов (убрала `int()`-краш при десинке).
