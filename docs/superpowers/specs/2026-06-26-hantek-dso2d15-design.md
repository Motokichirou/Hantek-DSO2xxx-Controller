...
> ✅ **РИСК РАЗРЕШЁН на железе (2026-06-27).** Команда чтения — **`PRIVate:WAVeform:DATA:ALL?`** (с префиксом `PRIVate:`! отдаёт ВСЕ каналы; обычный `:WAVeform:DATA:ALL?` отдаёт только CH1). Найдено через форум 4pda → GitHub `phmarek/hantek-dso2000` (тот же VID/PID 0x049F/0x505E).
> - Каждый пакет: `#9`+pkt_len[2:11]+total[11:20]+uploaded[20:29] + 128-байтовый meta, затем кусок сэмплов **с байта 128**. `total = N_кан × points`. Кусок пишется в общий буфер по позиции `uploaded`; читать пока собрано `total` байт.
> - **Каналы ИНТЕРЛИВНЫ блоками по 2000 байт** `|CH1|CH2|CH1|CH2|` → де-интерлив по индексу включённого канала.
> - **Сэмпл = signed int8, 25 counts/division.** **`volts = sample × (Vдел/25) − offset_volts`**; `t[i] = i/srate` (совпало с phmarek).
> - Meta-поля: enable [75:79], srate [79:88] (как во frozen reference §7). Смещения каналов в meta бинарны/ненадёжны — берём `:CHANnel<n>:OFFSet?`/`:SCALe?`.
> - Детали и реальные фикстуры: `tests/fixtures/waveform/FIXTURES.md`; реализация — `hantek_dso2d15/waveform/`.
> - **TODO:** поискать другие недокументированные команды с префиксом `PRIVate:*`.
...
1. ✅ **РАЗРЕШЕНО (2026-06-27):** кодировка/масштаб `WAVeform:DATA:ALL?` — signed int8, 25 counts/div, `volts=sample×Vдел/25−offset`, `t=i/srate`. Откалибровано петлёй DDS→CH1 (R²=0.995). Детали — §4.3 и `tests/fixtures/waveform/FIXTURES.md`.
