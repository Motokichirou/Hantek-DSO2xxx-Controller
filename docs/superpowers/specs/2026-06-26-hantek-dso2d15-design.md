...
> ✅ **РИСК РАЗРЕШЁН на железе (2026-06-27, петля DDS→CH1, R²=0.995).** Кодировка и масштаб откалиброваны:
> - Поток пакетов: HEADER (128 б ASCII) → по одному DATA-пакету на включённый канал (по возрастанию CH1→CH2), затем снова HEADER. Каждый пакет: `#9`+pkt_len[2:11]+total[11:20]+uploaded[20:29]+payload(`raw[29:]`). Читать DATA пока `uploaded<total`; `total = N_кан×points + 99`.
> - **Сэмпл = signed int8, 1 байт/сэмпл, 25 counts/division.**
> - **`volts = sample × (Vдел/25) − offset_volts`**; `t[i] = i/srate`.
> - Header-поля (выверенные индексы, отличаются от §7 reference на +4): chNoff[31+4k:35+4k] (signed counts), enable[79:83] (поканально '1'/'0'), srate[83:92]. Поле «voltage» заголовка = volts/count при полном разрешении АЦП (204.8/дел) — к экранным данным НЕ применять.
> - Детали и реальные фикстуры: `tests/fixtures/waveform/FIXTURES.md`; реализация — Layer 3 `waveform/` (план `docs/superpowers/plans/2026-06-27-waveform-decoder.md`).
